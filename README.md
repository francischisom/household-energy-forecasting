...
│
├── outputs/
│   ├── figures/                        # Generated plots (EDA, forecasts, importance)
│   ├── forecasts/                      # Per-model forecast CSVs
│   └── metrics/                        # Model comparison table, Part 9 answers
│
└── report/
...
Python 3.10+ with: `numpy`, `pandas`, `matplotlib`, `seaborn`, `scipy`,
`statsmodels`, `scikit-learn`, `xgboost`. Foundation models additionally need
`torch` and `chronos-forecasting` (the notebook falls back gracefully if these
are unavailable).
